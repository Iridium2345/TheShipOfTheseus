package data.missions.IrX_Test;
...
//		api.getDefaultCommander(FleetSide.PLAYER).getStats().setSkillLevel(Skills.ELECTRONIC_WARFARE, 3);
		...
		api.setFleetTagline(FleetSide.PLAYER, "IrX");
		api.setFleetTagline(FleetSide.ENEMY, "太空垃圾");
		
		// These show up as items in the bulleted list under 
		// "Tactical Objectives" on the mission detail screen
		api.addBriefingItem("____ 启动1");
		...
		for (String testID : Global.getSettings().getAllVariantIds()) {
			if (Global.getSettings().getVariant(testID).hasHullMod("IrX_quantized_hull")) {
